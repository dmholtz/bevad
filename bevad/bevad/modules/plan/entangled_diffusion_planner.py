...
class EntangledDiffusionPlanner(nn.Module):
    def __init__(
        self,
        # representation
        num_commands: int,
    ...
    def forward_train(
        ...
        **kwargs,
        ...
        norm_traj = normalize_trajectory(planning_traj)
        ...
        noisy_x = noisy_traj_emb + self.query_pe
        x_padding_mask = torch.logical_not(planning_mask)
        ...
        out, intermediate_losses = self.transformer(
            ...
            gt_path=planning_traj[..., :2],
            gt_path_mask=planning_mask,
        )
        xhat = out["q"]

        # decode
        pred_norm_traj = self.trajectory_decoder(xhat)
    ...
    def forward_test(
        ...
        **kwargs,
        ...
        x_padding_mask = (
            None if planning_mask is None else torch.logical_not(planning_mask)
        )
    ...
    def _compute_diffusion_loss(self, input_traj, target_traj, mask_traj):
        loss_traj = (
