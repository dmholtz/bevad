...
model = dict(
    ...
    planning_head=dict(
        type="EntangledDiffusionPlanner",
        num_commands=6,
        ...
        disentangled_decoder=dict(
            ...
            decoder_layer=dict(
                ...
                query_lengths=(planning_steps, 0),
            ),
        ),
        batch_multiplier=None,
        hidden_dim_time=512,
        timestep_resolution=10,
        cfg_p_uncond=0.5,
        scheduler_parameters=dict(
            num_train_timesteps=1000, # de-facto standard, affects alpha_bar computation
            beta_schedule="scaled_linear", # training + inference -> change requires re-training
            prediction_type="sample", # assumed by model (fixed)
            timestep_spacing="trailing", # inference only -> can be changed after training
            rescale_betas_zero_snr=True, # training + inference -> change requires re-training
        ),
        num_denoising_steps=10, # inference only -> can be changed after training
    ),
)
