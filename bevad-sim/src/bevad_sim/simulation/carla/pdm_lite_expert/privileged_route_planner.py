...
class PrivilegedRoutePlanner:
    ...
    def run_step(self, agent_position):
        ...
        self.route_index += np.argmin(
            np.linalg.norm(
                agent_position[None, :2] - self.route_points[self.route_index : search_range, :2],
                axis=1,
            )
        )
        ...
        for i in range(len(self.route_waypoints) - 2):
            ...
            if (
                ...
                while True:
                    ...
                    wp = (
                        self.route_waypoints[j].get_right_lane() if to_left else self.route_waypoints[j].get_left_lane()
                    )
    ...
    def compute_speed_limits(self, carla_map):
        ...
        file_name_speed_limits = f"{self.config.speed_limits_path}/{map_name}_speed_limits.npy"
    ...
    def compute_leading_vehicles(self, list_vehicles, ego_vehicle_id):
        ...
        if len(vehicle_ids) and self.route_index != self.route_points.shape[0]:
            ...
            rotation_angles = self.rotation_angles[self.route_index : self.route_index + max_distance][
                :: self.points_per_meter
            ]
