...
class LongitudinalPIDController(LongitudinalController):
    ...
    def get_throttle_and_brake(self, hazard_brake, target_speed, current_speed):
        ...
        derivative = (
            0 if len(self.speed_error_window) == 1 else self.speed_error_window[-1] - self.speed_error_window[-2]
        )
        integral = np.mean(self.speed_error_window)

        throttle = (
            self.proportional_gain * speed_error + self.derivative_gain * derivative + self.integral_gain * integral
        )
