...
import bevad_sim.simulation.carla.pdm_lite_expert.transfuser_utils as t_u
...
from bevad_sim.simulation.carla.pdm_lite_expert.config import GlobalConfig
from bevad_sim.simulation.carla.pdm_lite_expert.kinematic_bicycle_model import KinematicBicycleModel
from bevad_sim.simulation.carla.pdm_lite_expert.lateral_controller import LateralPIDController
from bevad_sim.simulation.carla.pdm_lite_expert.longitudinal_controller import LongitudinalLinearRegressionController
from bevad_sim.simulation.carla.pdm_lite_expert.nav_planner import RoutePlanner
from bevad_sim.simulation.carla.pdm_lite_expert.privileged_route_planner import PrivilegedRoutePlanner
from bevad_sim.simulation.carla.pdm_lite_expert.route_manipulation import downsample_route
from bevad_sim.simulation.carla.pdm_lite_expert.scenario_logger import ScenarioLogger


class PdmLiteAutopilot(BaseAgent):
    def __init__(self, config):
        ...
        CarlaDataProvider.active_scenarios = []  # TODO: patch srunner to support this
    ...
    def default_config(cls):
        ...
        config = {
            "verbose": True,
            "speed_limits_path": "artifacts/speed_limits",
        }
        return config
    ...
    def run_step(self, observation: CoreContainer) -> CoreContainer:
        ...
        control, driving_data = self._get_control(None, False)  # TODO: remove input_data=None, remove plant=False

        simulator_control = {
            "throttle": np.clip(control.throttle, 0, 1.0),
            "brake": np.clip(control.brake, 0, 1.0),
            "steer": np.clip(control.steer, -1.0, 1.0),
        }

        if observation.action is None:
            observation.action = Action.create_empty()
            observation.action.is_valid = np.ones((1, 1), dtype=bool)
        observation.action.simulator_control = [[simulator_control]]
        return observation

        # END PDM-lite #############################################

    def setup(self, path_to_conf_file=None, route_index=None, traffic_manager=None):
        """
        ...
        self.pdmconfig = GlobalConfig()
        self.pdmconfig.speed_limits_path = self.config["speed_limits_path"]
        ...
        # Dynamics models
        self.ego_model = KinematicBicycleModel(self.pdmconfig)
        self.vehicle_model = KinematicBicycleModel(self.pdmconfig)
        ...
        # Controllers
        self._turn_controller = LateralPIDController(self.pdmconfig)
        ...
        if os.environ.get("SAVE_PATH", None) is not None:
            ...
            self.lon_logger = ScenarioLogger(
                ...
                logging_freq=self.pdmconfig.logging_freq,
                log_only=True,
                route_only=False,  # with vehicles
                roi=self.pdmconfig.logger_region_of_interest,
            )
    ...
    def start_episode(self, env: Optional[Any] = None):
        ...
        self.set_global_plan(simulator.route_scenario.gps_route, simulator.route_scenario.route)
    ...
    def _init(self, hd_map):
        ...
        self._waypoint_planner = PrivilegedRoutePlanner(self.pdmconfig)
        ...
        self._longitudinal_controller = LongitudinalLinearRegressionController(self.pdmconfig)
        self._command_planner = RoutePlanner(
            self.pdmconfig.route_planner_min_distance,
            self.pdmconfig.route_planner_max_distance,
        )
    ...
    def tick_autopilot(self, input_data):
        ...
        rotation = np.deg2rad(self._vehicle.get_transform().rotation.yaw)
        compass = t_u.normalize_angle(rotation)
    ...
    def _get_control(self, input_data, plant):
        ...
        self.remaining_route = route_np[self.pdmconfig.tf_first_checkpoint_distance :][
            :: self.pdmconfig.points_per_meter
        ]
        self.remaining_route_original = self._waypoint_planner.original_route_points[
            self._waypoint_planner.route_index :
        ][self.pdmconfig.tf_first_checkpoint_distance :][:: self.pdmconfig.points_per_meter]

        # Get the current speed and target speed
        ego_speed = tick_data["speed"]
        target_speed = min(
            speed_limit * self.pdmconfig.ratio_target_speed_limit, 72.0 / 3.6
        )  # merge the two last speed bins

        # Reduce target speed if there is a junction ahead
        for i in range(min(self.pdmconfig.max_lookahead_to_check_for_junction, len(route_wp))):
            if route_wp[i].is_junction:
                target_speed = min(target_speed, self.pdmconfig.max_speed_in_junction)
                break
        ...
        control.steer = steer + self.pdmconfig.steer_noise * np.random.randn()
        control.throttle = throttle
        control.brake = float(brake or control_brake)

        # Apply brake if the vehicle is stopped to prevent rolling back
        if control.throttle == 0 and ego_speed < self.pdmconfig.minimum_speed_to_prevent_rolling_back:
            control.brake = 1
        ...
        if self.ego_blocked_for_ticks >= self.pdmconfig.max_blocked_ticks:
            control.throttle = 1
            control.brake = 0
    ...
    def _manage_route_obstacle_scenarios(self, target_speed, ego_speed, route_waypoints, list_vehicles, route_points):
        ...
        def compute_min_time_for_distance(distance, target_speed, ego_speed):
            ...
            while True:
                ...
                if remaining_distance - current_speed * self.pdmconfig.fps_inv < 0:
                    break

                remaining_distance -= current_speed * self.pdmconfig.fps_inv
                min_time_needed += self.pdmconfig.fps_inv

                # Values from kinematic bicycle model
                normalized_speed = current_speed / 120.0
                speed_change_params = self.pdmconfig.compute_min_time_to_cover_distance_params
        ...
        def get_previous_road_lane_ids(starting_waypoint):
            ...
            for _ in range(self.pdmconfig.previous_road_lane_retrieve_distance):
        ...
        def is_overtaking_path_clear(
            ...
            ego_distance = (
                ...
                + self.pdmconfig.check_path_free_safety_distance
            )
            ...
            for vehicle in list_vehicles:
                ...
                if (
                    ...
                    if other_vehicle_time < ego_time + self.pdmconfig.check_path_free_safety_time:
                        path_clear = False
                        break
        ...
        if len(CarlaDataProvider.active_scenarios) != 0:
            ...
            if scenario_type == "InvadingTurn":
                ...
                if closest_distance < self.pdmconfig.default_max_distance_to_process_scenario:
                ...
                if horizontal_distance < self.pdmconfig.default_max_distance_to_process_scenario:
                    transition_length = {
                        "Accident": self.pdmconfig.transition_smoothness_distance,
                        "ConstructionObstacle": self.pdmconfig.transition_smoothness_factor_construction_obstacle,
                        "ParkedObstacle": self.pdmconfig.transition_smoothness_distance,
                    }[scenario_type]
                ...
                # Shift the route around the obstacles
                if horizontal_distance < self.pdmconfig.default_max_distance_to_process_scenario and not changed_route:
                    transition_length = {
                        "AccidentTwoWays": self.pdmconfig.transition_length_accident_two_ways,
                        "ConstructionObstacleTwoWays": self.pdmconfig.transition_length_construction_obstacle_two_ways,
                        "ParkedObstacleTwoWays": self.pdmconfig.transition_length_parked_obstacle_two_ways,
                        "VehicleOpensDoorTwoWays": self.pdmconfig.transition_length_vehicle_opens_door_two_ways,
                    }[scenario_type]
                    add_before_length = {
                        "AccidentTwoWays": self.pdmconfig.add_before_accident_two_ways,
                        "ConstructionObstacleTwoWays": self.pdmconfig.add_before_construction_obstacle_two_ways,
                        "ParkedObstacleTwoWays": self.pdmconfig.add_before_parked_obstacle_two_ways,
                        "VehicleOpensDoorTwoWays": self.pdmconfig.add_before_vehicle_opens_door_two_ways,
                    }[scenario_type]
                    add_after_length = {
                        "AccidentTwoWays": self.pdmconfig.add_after_accident_two_ways,
                        "ConstructionObstacleTwoWays": self.pdmconfig.add_after_construction_obstacle_two_ways,
                        "ParkedObstacleTwoWays": self.pdmconfig.add_after_parked_obstacle_two_ways,
                        "VehicleOpensDoorTwoWays": self.pdmconfig.add_after_vehicle_opens_door_two_ways,
                    }[scenario_type]
                    factor = {
                        "AccidentTwoWays": self.pdmconfig.factor_accident_two_ways,
                        "ConstructionObstacleTwoWays": self.pdmconfig.factor_construction_obstacle_two_ways,
                        "ParkedObstacleTwoWays": self.pdmconfig.factor_parked_obstacle_two_ways,
                        "VehicleOpensDoorTwoWays": self.pdmconfig.factor_vehicle_opens_door_two_ways,
                    }[scenario_type]
                ...
                if (
                    ...
                    < self.pdmconfig.max_distance_to_overtake_two_way_scnearios
                    and not path_clear
                ):
                    ...
                    overtake_speed = (
                        self.pdmconfig.overtake_speed_vehicle_opens_door_two_ways
                        if scenario_type == "VehicleOpensDoorTwoWays"
                        else self.pdmconfig.default_overtake_speed
                    )
                ...
                if path_clear:
                    if (
                        self._waypoint_planner.route_index
                        >= to_index - self.pdmconfig.distance_to_delete_scenario_in_two_ways
                    ):
                        CarlaDataProvider.active_scenarios = CarlaDataProvider.active_scenarios[1:]
                    target_speed = {
                        "AccidentTwoWays": self.pdmconfig.default_overtake_speed,
                        "ConstructionObstacleTwoWays": self.pdmconfig.default_overtake_speed,
                        "ParkedObstacleTwoWays": self.pdmconfig.default_overtake_speed,
                        "VehicleOpensDoorTwoWays": self.pdmconfig.overtake_speed_vehicle_opens_door_two_ways,
                    }[scenario_type]
                    keep_driving = True
                else:
                    distance_to_leading_actor = (
                        float(from_index + 15 - self._waypoint_planner.route_index) / self.pdmconfig.points_per_meter
                    )
                    target_speed = self._compute_target_speed_idm(
                        ...
                        s0=self.pdmconfig.idm_two_way_scenarios_minimum_distance,
                        T=self.pdmconfig.idm_two_way_scenarios_time_headway,
                    )
                ...
                if (
                    horizontal_distance < self.pdmconfig.max_distance_to_process_hazard_at_side_lane_two_ways
                    ...
                    path_clear = is_overtaking_path_clear(
                        ...
                        ego_speed,
                        prev_road_lane_ids,
                        min_speed=self.pdmconfig.default_overtake_speed,
                    )

                    if path_clear:
                        transition_length = self.pdmconfig.transition_smoothness_distance
                ...
                if path_clear:
                    ...
                    target_speed, keep_driving = (
                        self.pdmconfig.default_overtake_speed,
                        True,
                    )
                ...
                if (
                    horizontal_distance < self.pdmconfig.max_distance_to_process_hazard_at_side_lane
                ...
                if horizontal_distance < self.pdmconfig.default_max_distance_to_process_scenario and not changed_route:
                    ...
                    from_index = self._waypoint_planner.route_index + 30 * self.pdmconfig.points_per_meter
                    to_index = from_index + int(2 * self.pdmconfig.points_per_meter) * self.pdmconfig.points_per_meter
        ...
        if self.visualize == 1:
            for i in range(
                min(
                    ...
                    self.pdmconfig.draw_future_route_till_distance,
                )
            ):
                loc = route_points[i]
                ...
                self._world.debug.draw_point(
                    location=loc,
                    size=0.05,
                    color=self.pdmconfig.future_route_color,
                    life_time=self.pdmconfig.draw_life_time,
                )
    ...
    def save(
        ...
        """
        frame = self.step // self.pdmconfig.data_save_freq
        ...
        remaining_route = self.remaining_route[: self.pdmconfig.num_route_points_saved]
        remaining_route_original = self.remaining_route_original[: self.pdmconfig.num_route_points_saved]
        ...
        if (self.step % self.pdmconfig.data_save_freq == 0) and (self.save_path is not None) and self.datagen:
    ...
    def _get_steer(self, route_points, current_position, current_heading, current_speed):
        ...
        """
        speed_scale = self.pdmconfig.lateral_pid_speed_scale
        speed_offset = self.pdmconfig.lateral_pid_speed_offset
        ...
        lookahead_distance = np.clip(
            lookahead_distance,
            self.pdmconfig.lateral_pid_default_lookahead,
            self.pdmconfig.lateral_pid_maximum_lookahead_distance,
        )
    ...
    def _compute_target_speed_idm(
        ...
        a = self.pdmconfig.idm_maximum_acceleration  # Maximum acceleration [m/s²]
        b = (
            self.pdmconfig.idm_comfortable_braking_deceleration_high_speed
            if ego_speed > self.pdmconfig.idm_comfortable_braking_deceleration_threshold
            else self.pdmconfig.idm_comfortable_braking_deceleration_low_speed
        )  # Comfortable deceleration [m/s²]
        delta = self.pdmconfig.idm_acceleration_exponent  # Acceleration exponent

        t_bound = self.pdmconfig.idm_t_bound
        ...
        def idm_equations(t, x):
            ...
            s = max(
                0.1,
                distance_to_leading_actor + t * leading_actor_speed - ego_position - leading_actor_length,
            )
    ...
    def is_near_lane_change(self, ego_velocity, route_points):
        ...
        ) + self.pdmconfig.braking_distance_calculation_safety_distance

        # Determine the number of waypoints to look ahead based on the braking distance
        look_ahead_points = max(
            self.pdmconfig.minimum_lookahead_distance_to_compute_near_lane_change,
            min(
                route_points.shape[0],
                self.pdmconfig.points_per_meter * int(braking_distance),
            ),
        )
        ...
        from_index = max(
            0,
            current_route_index - self.pdmconfig.check_previous_distance_for_lane_change,
        )
    ...
    def predict_other_actors_bounding_boxes(
        ...
        if not plant:
            ...
            nearby_actors = [
                ...
                and actor.get_location().distance(ego_vehicle_location) < self.pdmconfig.detection_radius
            ]
            ...
            if nearby_actors:
                ...
                for actor_idx, actor in enumerate(nearby_actors):
                    ...
                    for i in range(num_future_frames):
                        ...
                        s = (
                            self.pdmconfig.high_speed_min_extent_x_other_vehicle_lane_change
                            if near_lane_change
                            else self.pdmconfig.high_speed_min_extent_x_other_vehicle
                        )
                        extent.x *= (
                            self.pdmconfig.slow_speed_extent_factor_ego
                            if future_velocities[i, actor_idx]
                            < self.pdmconfig.extent_other_vehicles_bbs_speed_threshold
                            else max(
                                s,
                                self.pdmconfig.high_speed_min_extent_x_other_vehicle
                                * float(i)
                        ...
                        extent.y *= (
                            ...
                            < self.pdmconfig.extent_other_vehicles_bbs_speed_threshold
                                ...
                                self.pdmconfig.high_speed_min_extent_y_other_vehicle,
                                self.pdmconfig.high_speed_extent_y_factor_other_vehicle
                                * float(i)
                                / float(num_future_frames),
                            )
                        )
                    ...
                    for (
                        ...
                        for bb in actors_forecasted_bounding_boxes:
                            self._world.debug.draw_box(
                                ...
                                thickness=0.1,
                                color=self.pdmconfig.other_vehicles_forecasted_bbs_color,
                                life_time=self.pdmconfig.draw_life_time,
                            )
                ...
                if vehicle_id in leading_vehicle_ids and not near_lane_change:
                    ...
                    target_speed_wrt_leading_vehicle = min(
                        ...
                        self._compute_target_speed_idm(
                            ...
                            s0=self.pdmconfig.idm_leading_vehicle_minimum_distance,
                            T=self.pdmconfig.idm_leading_vehicle_time_headway,
                        ),
                    )
                ...
                for vehicle_id in predicted_bounding_boxes.keys():
                    ...
                    if vehicle_id in leading_vehicle_ids and not near_lane_change:
                        ...
                        self._world.debug.draw_box(
                            ...
                            rotation=bb.rotation,
                            thickness=0.5,
                            color=self.pdmconfig.leading_vehicle_color,
                            life_time=self.pdmconfig.draw_life_time,
                        )
                        ...
                        self._world.debug.draw_box(
                            ...
                            rotation=bb.rotation,
                            thickness=0.5,
                            color=self.pdmconfig.trailing_vehicle_color,
                            life_time=self.pdmconfig.draw_life_time,
                        )
    ...
    def compute_target_speeds_wrt_all_actors(
        ...
        hazard_color = self.pdmconfig.ego_vehicle_forecasted_bbs_hazard_color
        normal_color = self.pdmconfig.ego_vehicle_forecasted_bbs_normal_color
        color = normal_color
        ...
        for i, ego_bounding_box in enumerate(ego_bounding_boxes):
            for vehicle_id, bounding_boxes in predicted_bounding_boxes.items():
                ...
                if (
                    vehicle_id in leading_vehicle_ids
                    and not near_lane_change
                        ...
                        if (
                            ...
                            target_speed_bicycle = min(
                                ...
                                self._compute_target_speed_idm(
                                    ...
                                    s0=self.pdmconfig.idm_bicycle_minimum_distance,
                                    T=self.pdmconfig.idm_bicycle_desired_time_headway,
                                ),
                            )
                ...
                if self.check_obb_intersection(ego_bounding_box, pedestrian_bb[i]):
                    ...
                    target_speed_pedestrian = min(
                        ...
                        self._compute_target_speed_idm(
                            ...
                            distance_to_leading_actor=distance_to_actor,
                            s0=self.pdmconfig.idm_pedestrian_minimum_distance,
                            T=self.pdmconfig.idm_pedestrian_desired_time_headway,
                        ),
                    )
    ...
    def get_brake_and_target_speed(
        ...
        if self.visualize == 1:
            self._world.debug.draw_box(
                ...
                thickness=0.1,
                color=self.pdmconfig.ego_vehicle_bb_color,
                life_time=self.pdmconfig.draw_life_time,
            )
        ...
        num_future_frames = int(
            self.pdmconfig.bicycle_frame_rate
            * (
                self.pdmconfig.forecast_length_lane_change
                if near_lane_change
                else self.pdmconfig.default_forecast_length
            )
        )
    ...
    def forecast_ego_agent(
        ...
        for _ in range(num_future_frames):
            ...
            extent.x *= (
                self.pdmconfig.slow_speed_extent_factor_ego
                if current_ego_speed < self.pdmconfig.extent_ego_bbs_speed_threshold
                else self.pdmconfig.high_speed_extent_factor_ego_x
            )
            extent.y *= (
                self.pdmconfig.slow_speed_extent_factor_ego
                if current_ego_speed < self.pdmconfig.extent_ego_bbs_speed_threshold
                else self.pdmconfig.high_speed_extent_factor_ego_y
            )
    ...
    def forecast_walkers(self, actors, ego_vehicle_location, number_of_future_frames):
        ...
        pedestrians = [
            ...
            if ped.get_location().distance(ego_vehicle_location) < self.pdmconfig.detection_radius
        ]
        ...
        pedestrian_speeds = np.maximum(pedestrian_speeds, self.pdmconfig.min_walker_speed)
        ...
        for i, ped in enumerate(pedestrians):
            ...
            extent = bb.extent
            extent.x = max(self.pdmconfig.pedestrian_minimum_extent, extent.x)  # Ensure a minimum width
            extent.y = max(self.pdmconfig.pedestrian_minimum_extent, extent.y)  # Ensure a minimum length
            ...
            for bbs in nearby_pedestrians_bbs:
                for bbox in bbs:
                    self._world.debug.draw_box(
                        ...
                        thickness=0.1,
                        color=self.pdmconfig.pedestrian_forecasted_bbs_color,
                        life_time=self.pdmconfig.draw_life_time,
                    )
    ...
    def ego_agent_affected_by_red_light(
        ...
        for light, center, waypoints in self.list_traffic_lights:
            ...
            if center_loc.distance(ego_vehicle_location) > self.pdmconfig.light_radius:
                continue
            ...
            for wp in waypoints:
                ...
                if self.visualize == 1:
                    ...
                    self._world.debug.draw_point(
                        ...
                        size=0.1,
                        color=color,
                        life_time=(1.0 / self.pdmconfig.carla_fps) + 1e-6,
                    )
        ...
        # determine distance to traffic light based on region
        if "Town01" in self.world_map.name or "Town02" in self.world_map.name:
            # these towns have traffic lights placed before the intersection
            # it requires a larger distance to still be able to see the light box when stopped
            s0 = self.pdmconfig.idm_red_light_minimum_distance_europe
        else:
            # these towns have traffic lights placed after the intersection
            s0 = self.pdmconfig.idm_red_light_minimum_distance
        ...
        target_speed = self._compute_target_speed_idm(
            ...
            s0=s0,
            T=self.pdmconfig.idm_red_light_desired_time_headway,
        )
    ...
    def ego_agent_affected_by_stop_sign(
        ...
        if distance_to_stop_sign > self.pdmconfig.unclearing_distance_to_stop_sign:
            self.cleared_stop_sign = False
            self.waiting_ticks_at_stop_sign = 0
        elif ego_vehicle_speed < 0.1 and distance_to_stop_sign < self.pdmconfig.clearing_distance_to_stop_sign:
        ...
        target_speed = self._compute_target_speed_idm(
            ...
            s0=self.pdmconfig.idm_stop_sign_minimum_distance,
            T=self.pdmconfig.idm_stop_sign_desired_time_headway,
        )
    ...
    def check_obb_intersection(self, obb1, obb2):
        ...
        if (
            self.get_separating_plane(relative_position, obb1.rotation.get_forward_vector(), obb1, obb2)
            or self.get_separating_plane(relative_position, obb1.rotation.get_right_vector(), obb1, obb2)
            or self.get_separating_plane(relative_position, obb1.rotation.get_up_vector(), obb1, obb2)
            or self.get_separating_plane(relative_position, obb2.rotation.get_forward_vector(), obb1, obb2)
            or self.get_separating_plane(relative_position, obb2.rotation.get_right_vector(), obb1, obb2)
            or self.get_separating_plane(relative_position, obb2.rotation.get_up_vector(), obb1, obb2)
        ):
            return False
