...
class YieldToEmergencyVehicle(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, debug_mode=False, criteria_enable=True,
        ...
        self._ev_idle_time = 10  # [s]

        # km/h. How much the EV is expected to be faster than the EGO
        self._speed_increment = get_value_parameter(config, 'speed_increment', float, 25) # How much the actor will be faster then the reference_actor. [m/s]

        self._trigger_distance = get_value_parameter(config, 'trigger_distance', float, 50) # Distance between the actors [m]
    ...
    def _initialize_actors(self, config):
        ...
        CarlaDataProvider.active_scenarios.append((type(self).__name__, [actor, None, False, 1e9, 1e9, False]))
