...
from srunner.tools.scenario_helper import get_waypoint_in_distance, get_value_parameter
...
class ControlLoss(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        self._end_distance = 110 # [m]

        # Friction loss tends to have a much stronger steering compoenent then a throttle one
        self._throttle_mean = get_value_parameter(config, 'throttle mean', float, 0.03)
        self._throttle_std = get_value_parameter(config, 'throttle_std', float, 0.01)
        self._steer_mean = get_value_parameter(config, 'steer_mean', float, 0.055)
        self._steer_std = get_value_parameter(config, 'steer_std', float, 0.015)

        self._trigger_dist = get_value_parameter(config, 'trigger_dist', float, 2) # [m]
