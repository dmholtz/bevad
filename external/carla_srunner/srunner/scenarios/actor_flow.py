...
class NonSignalizedJunctionTurnRightSimple(EnterActorFlow):
    pass

class EnterActorTwoWayFlow(BasicScenario):
    """
    Scenario where the ego turns left into a two‑way flow: 
    vehicles coming from the right (which ego must yield to and then enter in front of)
    and vehicles coming from the left (which ego must also yield to).
    """

    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
                 timeout=180):
        """
        Setup parameters for two‑way actor flow scenario.
        Expects config.other_parameters to contain:
          - 'start_left_flow', 'end_left_flow'
          - 'start_right_flow', 'end_right_flow'
        """
        self._world = world
        self._map = CarlaDataProvider.get_map()
        self.timeout = timeout
        ...
        ego_location = config.trigger_points[0].location
        self._reference_waypoint = self._map.get_waypoint(ego_location)

        # Left-to-right flow parameters
        self._start_left_flow = convert_dict_to_location(config.other_parameters['start_left_flow'])
        self._end_left_flow   = convert_dict_to_location(config.other_parameters['end_left_flow'])
        # Right-to-left flow parameters
        self._start_right_flow = convert_dict_to_location(config.other_parameters['start_right_flow'])
        self._end_right_flow   = convert_dict_to_location(config.other_parameters['end_right_flow'])

        # common flow settings
        self._flow_speed = get_value_parameter(config, 'flow_speed', float, 10)
        self._source_dist_interval = get_interval_parameter(config, 'source_dist_interval', float, [20, 50])
        self._sink_distance = get_value_parameter(config, 'sink_distance', float, 2.0)

        super().__init__("EnterActorTwoWayFlow",
                         ego_vehicles,
                         config,
                         world,
                         debug_mode,
    ...
    def _create_behavior(self):
        """
        Build behavior tree: spawn two actor flows (left and right)
        and trigger when ego is within sink_distance of their end-points.
        """
        # get waypoints for both flows
        left_src_wp  = self._map.get_waypoint(self._start_left_flow)
        left_sink_wp = self._map.get_waypoint(self._end_left_flow)
        right_src_wp  = self._map.get_waypoint(self._start_right_flow)
        right_sink_wp = self._map.get_waypoint(self._end_right_flow)

        # lanes for each direction
        left_source_wps  = get_same_dir_lanes(left_src_wp)
        left_sink_wps    = get_same_dir_lanes(left_sink_wp)
        right_source_wps = get_same_dir_lanes(right_src_wp)
        right_sink_wps   = get_same_dir_lanes(right_sink_wp)

        # Parallel: keep spawning and monitoring both flows until one triggers or timeout
        root = py_trees.composites.Parallel(policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)

        # spawn left‑to‑right flow (ego must yield to these)
        for src_wp, sink_wp in zip(left_source_wps, left_sink_wps):
            root.add_child(InTriggerDistanceToLocation(self.ego_vehicles[0],
        ...
        # spawn right‑to‑left flow (ego will turn into these)
        for src_wp, sink_wp in zip(right_source_wps, right_sink_wps):
            root.add_child(InTriggerDistanceToLocation(self.ego_vehicles[0],
                                                       sink_wp.transform.location,
                                                       self._sink_distance))
            root.add_child(ActorFlow(
        ...
        # overall timeout
        root.add_child(ScenarioTimeout(self.timeout, self.config.name))

        # wrap in sequence to handle junction clearing and route mode toggles
        sequence = py_trees.composites.Sequence()
        if self.route_mode:
            # clear the intersection before spawning
            sequence.add_child(HandleJunctionScenario(
                clear_junction=True,
                clear_ego_entry=True,
                remove_entries=left_source_wps + right_source_wps,
                remove_exits=[],
                stop_entries=False,
                extend_road_exit=20.0
            ))
            sequence.add_child(SwitchRouteSources(False))
    ...
    def _create_test_criteria(self):
        """
        Criteria include collision with any actor and timeout.
        """
        criteria = [ScenarioTimeoutTest(self.ego_vehicles[0], self.config.name),
    ...
    def __del__(self):
        """
        Cleanup all spawned actors and traffic lights.
        """
        self.remove_all_actors()
...
class NonSignalizedJunctionLeftTurnEnterActorFlow(EnterActorTwoWayFlow):
    pass
