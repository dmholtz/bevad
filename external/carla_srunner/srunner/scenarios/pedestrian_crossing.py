...
class PedestrianCrossing(BasicScenario):
    ...
    def _initialize_actors(self, config):
        ...
        for i, walker_data in enumerate(self._walker_data):
            ...
            if walker is None:
                print(f"Failed to spawn adversary number {i}. Keep trying to spawn other adversaries.")
                continue
                
                # the srunner would fail the scenario completely, we relax it and keep all walkers spawned so far
            ...
            self._actual_walker_data.append(walker_data)
        ...
        # only fail if no walker was spawned
        if len(self.other_actors) < 1:
            raise ValueError("Failed to spawn any adversary")
    ...
    def _create_behavior(self):
        ...
        for walker_actor, walker_data in zip(self.other_actors, self._actual_walker_data):
