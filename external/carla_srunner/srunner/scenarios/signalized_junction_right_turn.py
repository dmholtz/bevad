...
from srunner.scenariomanager.scenarioatomics.atomic_behaviors import ActorFlow, TrafficLightAwareActorFlow, TrafficLightFreezer, ScenarioTimeout
...
class JunctionRightTurn(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        self._green_light_delay = get_value_parameter(config, 'green_light_delay', float, 5) # Wait before the ego's lane traffic light turns green [s]
        self._flow_tl_dict = {}
        self._init_tl_dict = {}
        self._end_distance = 10 # Distance after the junction before the scenario ends [m]

        default_flow_speed = random.uniform(7, 20)
        min_flow_distance = random.uniform(10, 30)
        max_flow_distance = random.uniform(35, 70)

        self._flow_speed = get_value_parameter(config, 'flow_speed', float, default_flow_speed)
        self._source_dist_interval = get_interval_parameter(config, 'source_dist_interval', float, [min_flow_distance, max_flow_distance]) # [m]
        self._scenario_timeout = 240 # [s]
...
class SignalizedJunctionRightTurn(JunctionRightTurn):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
                 timeout=80):
        # Optional parameter to set traffic lights delayed compared to trigger point
        # If we dont set trafic lights will turn red immediately, thus implementing old behaviour
        self._switch_traffic_light_after_distance = get_value_parameter(
            config, "switch_traffic_light_after_distance", float, 0
        ) 
    ...
    def _initialize_actors(self, config):
        ...
        # --------------------------------------------
        # Helper to create a starting scenario that has valid tc states and enables resetting it later
        self.traffic_light_info_scenario_start = {}

        def force_set_tl(tl, state):
            self.traffic_light_info_scenario_start[tl] = {
                "state": tl.get_state(),
            ...
            tl.set_state(state)
            tl.set_green_time(LONG_WAIT_TIME + elapsed_time)
            tl.set_red_time(LONG_WAIT_TIME + elapsed_time)
            tl.set_yellow_time(LONG_WAIT_TIME + elapsed_time)
        ...
        for tl in tls:
            if tl.id == ego_tl.id:
                ...
                # Car starts with free drive
                force_set_tl(tl, carla.TrafficLightState.Green)
                ...
                # Non flow starts blocked
                force_set_tl(tl, carla.TrafficLightState.Red)
    ...
    def _create_behavior(self):
        ...
        tl_freezer_sequence.add_child(
            DriveDistance(
                self.ego_vehicles[0], self._switch_traffic_light_after_distance
            )
        )
    ...
    def terminate(self):
        ...
        for tl in self.traffic_light_info_scenario_start:
            tl.set_state(self._previous_traffic_light_info[tl]["state"])
            tl.set_green_time(
                self._previous_traffic_light_info[tl]["green_time"])
            tl.set_red_time(self._previous_traffic_light_info[tl]["red_time"])
            tl.set_yellow_time(
                self._previous_traffic_light_info[tl]["yellow_time"])
...
class NonSignalizedJunctionRightTurnStrict(NonSignalizedJunctionRightTurn):
    pass

class SignalizedJunctionRightTurnV2(JunctionRightTurn):
    """
    Signalized version of JunctionRightTurn.

    It is a V2 because compared to the original code it fixes, that when turning right,
    we must not have any flow coming from left.
    """
    ...
    def __init__(
        ...
        self._switching_tl_dict = {}
        self.red_green_switch_time = 2
    ...
    def _initialize_actors(self, config):
        ...
        for tl in tls:
            if tl.id == ego_tl.id:
                self._flow_tl_dict[tl] = carla.TrafficLightState.Green
                self._switching_tl_dict[tl] = carla.TrafficLightState.Red
                self._init_tl_dict[tl] = carla.TrafficLightState.Red
                ...
                # When we haev greed, it auomatically means that flow from left 
                # Should have red
                force_set_tl(tl, carla.TrafficLightState.Red)
            else:
                self._flow_tl_dict[tl] = carla.TrafficLightState.Red
    ...
    def _create_behavior(self):
        ...
        root.add_child(ScenarioTimeout(
            self._scenario_timeout, self.config.name))
        ...
        tl_freezer_sequence.add_child(
            TrafficLightFreezer(self._switching_tl_dict,
                                duration=self.red_green_switch_time)
        )
        tl_freezer_sequence.add_child(TrafficLightFreezer(self._flow_tl_dict))
        root.add_child(tl_freezer_sequence)
