...
class StaticCutIn(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True, timeout=60):
        ...
        self._reaction_time = get_value_parameter(config, 'reaction_time', float, 2.7) # Time the agent has to react to avoid the collision [s]
        ...
        self._speed = get_value_parameter(config, 'speed', float, 60) # [km/h]

        self._adversary_end_distance = 70

        self._extra_space = 30  # Leave extra space as a vehicle is invading the ego's lane (BA parameter) [m]
        ...
        self._blocker_distance = get_value_parameter(config, 'distance', float, 100) # [m]
