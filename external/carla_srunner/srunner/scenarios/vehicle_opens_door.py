...
class VehicleOpensDoorTwoWays(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        self._min_trigger_dist = 10 # Min distance that triggers the door opening [m]
        self._reaction_time = get_value_parameter(config, 'reaction_time', float, 3.0) # Time the agent has to react [s]

        self._opposite_wait_duration = 5 # [s]
        self._end_distance = 50 # [m]

        self._parked_distance = get_value_parameter(config, 'distance', float, 50) # [m]
        self._direction = get_value_parameter(config, 'direction', str, 'right') # 'left' or 'right'
        ...
        self._max_speed = get_value_parameter(config, 'speed', float, 60) # [km/h]
    ...
    def _initialize_actors(self, config):
        ...
        # add actors, that are relevant for PDM-lite export to the list active_scenarios
        CarlaDataProvider.active_scenarios.append((type(self).__name__, [self._parked_actor, None, self._direction, False, 1e9, 1e9, False]))
