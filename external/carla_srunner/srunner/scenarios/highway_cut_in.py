...
from srunner.tools.scenario_helper import generate_target_waypoint, get_value_parameter
...
class HighwayCutIn(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        self._same_lane_time = get_value_parameter(config, 'same_lane_time', float, 0.3) # Amount of time spent at the same lane before cutting in. [s]
        self._other_lane_time = get_value_parameter(config, 'other_lane_time', float, 3) # Amount of time spent at the other lane after cutting in. [s]
        self._change_time = get_value_parameter(config, 'change_time', float, 2) # Amount of time spent changing into the other [s]
        self._speed_perc = get_value_parameter(config, 'speed_perc', float, 80) # Percentage of the reference actor speed on which the cut in is performed. [%]
