...
class DynamicObjectCrossing(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True, timeout=60):
        ...
        self._distance = get_value_parameter(config, 'distance', float, 40) # >30m is required for PDM-lite to react properly
        default_blocker_model = random.choice(['static.prop.vendingmachine', "static.prop.container", "static.prop.advertisement"])
        self._blocker_model = get_value_parameter(config, 'blocker_model', str, default_blocker_model)
