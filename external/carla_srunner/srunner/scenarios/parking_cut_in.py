...
class ParkingCutIn(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True, timeout=60):
        ...
        self._cut_in_distance = get_value_parameter(config, 'cut_in_distance', float, 35) # [m]
        self._blocker_distance = get_value_parameter(config, 'blocker_distance', float, 28) # [m]

        self._adversary_speed = get_value_parameter(config, 'adversary_speed', float, 13.0) # Speed of the adversary [m/s]
        self._reaction_time = get_value_parameter(config, 'reaction_time', float, 2.35) # Time the agent has to react to avoid the collision [s]
