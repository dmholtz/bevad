...
class VehicleTurningRoutePedestrianStrictOnlySpwanRightSide(BasicScenario):

    """
    Similar to normal VehichleTurningRoutePedestrian, just that this one guaranteed that 
    pedestrians are only spawned at the right side of the road, thus increasing difficulty
    """
    _subtype = None

    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
                 timeout=60, name="VehicleTurningRoutePedestrianStrictOnlySpwanRightSide"):
        """
        Setup all relevant parameters and create scenario
        """
    ...
    def _initialize_actors(self, config):
        ...
        self._adversary_distance = right_wp.transform.location.distance(left_wp.transform.location)
        spawn_wp = right_wp  # Always spawn adversary on right side
