...
class TrafficLightAwareActorFlow(AtomicBehavior):
    """
    Advanced version of actor flow, unlike the normal one, this one accepts parameters of whether
    to adhere to traffic_lights and traffic_signs or not.
    """
    def __init__(
        self,
        source_wp,
        ...
        ignore_traffic_signs=True,
        name="ActorFlowIgnoreVehicles",
    ):
        super().__init__(name)
        # world, TM and blueprint
        self._world = CarlaDataProvider.get_world()
        self._tm = CarlaDataProvider.get_client().get_trafficmanager(
            CarlaDataProvider.get_traffic_manager_port()
        )
        ...
        self._source_wp = source_wp
        self._sink_wp = sink_wp
        self._source_transform = source_wp.transform
        self._source_location = self._source_transform.location
        self._sink_location = sink_wp.transform.location
        self._sink_dist = sink_dist
        self._speed = actor_speed
        self._initial_actors = initial_actors
        self._ignore_traffic_lights = ignore_traffic_lights
        self._ignore_traffic_signs = ignore_traffic_signs
        self._min_spawn_dist, self._max_spawn_dist = spawn_dist_interval
        self._rng = CarlaDataProvider.get_random_seed()
        self._spawn_dist = self._rng.uniform(self._min_spawn_dist, self._max_spawn_dist)
    ...
    def _spawn_actor(self, transform):
        ...
        # start by ignoring other vehicles (guarantee crash)
        self._tm.ignore_vehicles_percentage(actor, 100)
        # set traffic light and sign behavior
        self._tm.ignore_lights_percentage(
            actor, 100 if self._ignore_traffic_lights else 0
        )
        self._tm.ignore_signs_percentage(
            actor, 100 if self._ignore_traffic_signs else 0
        )

        # attach collision sensor to reset behavior on first hit
        sensor = self._world.spawn_actor(
            self._collision_bp,
            carla.Transform(),
            attach_to=actor
        )
        sensor.listen(lambda event, a=actor, s=sensor: self._on_collision(a, s))

        # register actor and its sensor
        self._actors.append(actor)
        self._collision_sensors.append(sensor)

    def _on_collision(self, actor, sensor):
        # On first collision, resume normal TM vehicle avoidance
        self._tm.ignore_vehicles_percentage(actor, 0)
        # remove and destroy this sensor
        try:
            sensor.stop()
            sensor.destroy()
        except RuntimeError:
            pass
        if sensor in self._collision_sensors:
            self._collision_sensors.remove(sensor)

    def update(self):
        # remove actors that reached sink
        ...
        # spawn new actor if needed
        if not self._actors:
            distance = self._spawn_dist + 1.0
        else:
            last_loc = CarlaDataProvider.get_location(self._actors[-1])
            distance = self._source_location.distance(last_loc) if last_loc else 0.0
    ...
    def terminate(self, new_status):
        if self._terminated:
            return
        self._terminated = True
        for sensor in list(self._collision_sensors):
            try:
                sensor.stop()
                sensor.destroy()
            except RuntimeError:
...
class OpenVehicleDoor(AtomicBehavior):
    ...
    def initialise(self):
        ...
        CarlaDataProvider.open_doors[self._actor.id] = True
        super().initialise()
