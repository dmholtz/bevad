...
def get_value_parameter(config, name, p_type, default):
    ...
    Args:
        config: Configuration object containing 'other_parameters'.
        name (str): Name of the parameter to retrieve.
        p_type (type): Expected type of the parameter value.
        default: Default value if the parameter is not found.

    Returns:
        The parameter value converted to the specified type, or the default value if not present.
    """
...
def get_interval_parameter(config, name, p_type, default):
    """
    Retrieves an interval parameter from the configuration.

    Args:
        config: Configuration object containing 'other_parameters'.
        name (str): Name of the parameter to retrieve.
        p_type (type): Expected type of the interval values.
        default: Default value if the parameter is not found.

    Returns:
        list: A list with two values [from, to] converted to the specified type, 
...
def generate_target_waypoint_in_route(waypoint, route):
    ...
    if target_waypoint is None:
        target_waypoint = route[-1][0].location
